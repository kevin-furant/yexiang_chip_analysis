...
def load_data_files(args):
    ...
    return project_info, alignment_df, buhuo_stats_df, sample_stats_df, snp_stats_df

def prepare_template_data(project_info, alignment_df, buhuo_stats_df, sample_stats_df, snp_stats_df, args):
    """准备模板渲染所需的所有数据"""
    ...
    for _, row in sample_stats_df.head(100).iterrows():
        # 计算总位点数
        total_sites = int(row.get('Ref_number', 0) + row.get('Het_number', 0) + 
                         row.get('Hom_number', 0) + row.get('NA_number', 0))
...
def main():
    ...
    try:
        # 1. 加载数据文件
        print("📂 加载数据文件...")
        project_info, alignment_df, buhuo_stats_df, sample_stats_df, snp_stats_df = load_data_files(args)
        
        # 2. 准备模板数据
        template_data = prepare_template_data(
            project_info, alignment_df, buhuo_stats_df, sample_stats_df, snp_stats_df, args
        )
