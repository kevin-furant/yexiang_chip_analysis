...
with open(input_samplelist) as samplelist, open(bwamem_file, 'w') as bwamem_shell, open(work_shell_file, "w") as work_shell, open(gatk_file, "w") as gatk_shell, open(stat_file, "w") as stat_shell, open(report_file, "w") as report_shell:
    ...
    for sample_name in sample_dict.keys():
        ...
        bwa_shell = f"""{bwa} mem -t {cpu} -R '@RG\\tID:{sample_name}\\tPL:illumina\\tSM:{sample_name}' {ref_genome} {read1} {read2} | {samtools} sort -@ {cpu_half} -m 2G --output-fmt BAM -o {bam_dir}/{sample_name}.sorted.bam && {samtools} index -@ {cpu} {bam_dir}/{sample_name}.sorted.bam && {samtools} quickcheck {bam_dir}/{sample_name}.sorted.bam && {samtools} view -b -f 4 -@ {cpu} {bam_dir}/{sample_name}.sorted.bam > {unmap_dir}/{sample_name}.unmap.bam && {samtools} stat --coverage 1,100,1 -@ {cpu} {bam_dir}/{sample_name}.sorted.bam > {stat_dir}/{sample_name}.bwa.stat && {sambamba} markdup -r -t {int(cpu/2)} -p --tmpdir {tmpdir} {bam_dir}/{sample_name}.sorted.bam {bam_dir}/{sample_name}.sorted.rmdup.bam
